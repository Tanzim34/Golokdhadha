...
public class stdnotify extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fetchData(teacherCollectionRef, dataList -> {
            ...
            binding.listview.setOnItemClickListener((parent, view, position, id) -> {
                ...
                //for requests
                if(type==1) {
                    Intent intent = new Intent(stdnotify.this, acceptRequest.class);
                ...
                if(type==2)
                {
                    Intent intent = new Intent(stdnotify.this, teachpayment.class);
                    intent.putExtra("student_id", studentID);
                    intent.putExtra("teacher_id", teacherID);
                }
                // for calender
                if(type==3){
                    Intent intent = new Intent(stdnotify.this, teachcalender.class);
                    intent.putExtra("student_id",studentID);
                    intent.putExtra("teacher_id", teacherID);
                }
                if(type==-1) Toast.makeText(stdnotify.this,"There is some error", Toast.LENGTH_SHORT).show();
            });
        });
    }
...
//                            });
    ...
    public void updateUI(ArrayList<Listdata> dataList) {
        // Update the UI with the fetched data
        listAdapter = new itemAdapter(stdnotify.this, dataList);
